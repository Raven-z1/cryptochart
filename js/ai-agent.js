/**
 * OpenRouter AI Trading Agent Integration
 * Connects to OpenRouter (OpenAI-compatible AI Gateway) to analyze Binance 5m USDⓈ-M charts,
...
// =========================================================================
// 2. OPENROUTER API CLIENT & PROMPT GENERATION
// =========================================================================
...
/**
 * Sends request to OpenRouter OpenAI-compatible AI Gateway
 */
async function callOpenRouter(systemPrompt, userPrompt) {
  const current = (typeof getOpenRouterInputValues === 'function') ? getOpenRouterInputValues() : {};
  const rawBase = (state.aiAgent.gatewayUrl || current.base || 'https://openrouter.ai/api/v1').trim();
  const apiKey = (state.aiAgent.apiKey || current.key || '').trim();
  const model = (state.aiAgent.model || current.model || 'openai/gpt-4o-mini').trim();

  if (!apiKey) {
    throw new Error('OpenRouter API Key is missing. Please configure your API Key in the settings below.');
  }
  ...
  try {
    ...
    throw new Error(`Failed to connect to OpenRouter at ${endpoint}. Check your internet connection and gateway URL.`);
  }
  ...
  if (res.status === 400) {
    ...
    if (errBody.includes('response_format') || errBody.includes('json_object')) {
      ...
      throw new Error(errJson && errJson.error ? errJson.error.message : `OpenRouter HTTP 400: ${errBody.slice(0, 150)}`);
    }
  }

  if (!res.ok) {
    let errMessage = `OpenRouter API error (HTTP ${res.status})`;
    try {
  ...
  if (!data.choices || !data.choices.length || !data.choices[0].message) {
    throw new Error('OpenRouter returned an empty or invalid response format.');
  }
...
function parseAiResponse(rawContent, snapshot) {
  ...
  return {
    ...
    strategy: result.strategy || 'OpenRouter AI Setup',
    rationale: result.rationale || 'Technical structure and momentum setup generated by OpenRouter AI.',
...
async function runAiTradeAnalysis(actionType = 'auto', customQuery = '') {
  ...
  if (!apiKey && typeof getOpenRouterInputValues === 'function') {
    const current = getOpenRouterInputValues();
    if (current.key) {
      apiKey = current.key;
      saveOpenRouterConfig(current.base, current.key, current.model, false);
    }
  }

  if (!apiKey) {
    showToast('🔑 OpenRouter API Key required. Please configure below.');
    if (dom.aiConfigCard) dom.aiConfigCard.classList.remove('hidden');
    const inputToFocus = document.getElementById('ai-key-input') || document.getElementById('openrouter-modal-key-input');
  ...
  try {
    ...
    updateAiLoadingStep('Sending market data to OpenRouter AI gateway...');
    const rawAiOutput = await callOpenRouter(systemPrompt, userPrompt);
    ...
    showToast(`✨ OpenRouter AI: ${plan.signal} Setup Plotted!`);
  } catch (err) {
...
function updateAiUiLoadingState(isLoading, actionType = 'auto') {
  ...
  if (isLoading) {
    ...
    if (dom.aiLoadingSubtext) {
      dom.aiLoadingSubtext.textContent = 'Calculating ATR, RSI, EMAs, and querying OpenRouter AI...';
    }
  }
...
function updateGatewayStatusBadge() {
  ...
  const model = state.aiAgent.model || 'openai/gpt-4o-mini';

  if (dom.aiGatewayStatusDot) {
    dom.aiGatewayStatusDot.className = `ai-dot ${key ? 'configured' : 'warning'}`;
  }
  if (dom.aiGatewayStatusText) {
    dom.aiGatewayStatusText.textContent = key ? 'OpenRouter Stored in Browser' : 'API Key Required';
  }
...
// Helper to retrieve current OpenRouter inputs from DOM, state, or localStorage
function getOpenRouterInputValues() {
  const base = (
    (document.getElementById('openrouter-modal-base-input') && document.getElementById('openrouter-modal-base-input').value) ||
    ...
    (function () { try { return localStorage.getItem('cc_openrouter_base') || ''; } catch (_) { return ''; } })() ||
    'https://openrouter.ai/api/v1'
  ).trim();

  const key = (
    (document.getElementById('openrouter-modal-key-input') && document.getElementById('openrouter-modal-key-input').value) ||
    (document.getElementById('ai-key-input') && document.getElementById('ai-key-input').value) ||
    state.aiAgent.apiKey ||
    (function () { try { return localStorage.getItem('cc_openrouter_key') || ''; } catch (_) { return ''; } })() ||
    ''
  ).trim();

  const model = (
    (document.getElementById('openrouter-modal-model-input') && document.getElementById('openrouter-modal-model-input').value) ||
    (document.getElementById('ai-model-input') && document.getElementById('ai-model-input').value) ||
    state.aiAgent.model ||
    (function () { try { return localStorage.getItem('cc_openrouter_model') || ''; } catch (_) { return ''; } })() ||
    'openai/gpt-4o-mini'
  ).trim();
...
// Synchronize OpenRouter configuration inputs across modals
function syncOpenRouterInputs() {
  const current = getOpenRouterInputValues();
  ...
  const openrouterBase = document.getElementById('openrouter-modal-base-input');
  const aiKey = document.getElementById('ai-key-input');
  const openrouterKey = document.getElementById('openrouter-modal-key-input');
  const aiModel = document.getElementById('ai-model-input');
  const openrouterModel = document.getElementById('openrouter-modal-model-input');

  if (aiBase && aiBase.value !== base) aiBase.value = base;
  if (openrouterBase && openrouterBase.value !== base) openrouterBase.value = base;
  if (aiKey && aiKey.value !== key) aiKey.value = key;
  if (openrouterKey && openrouterKey.value !== key) openrouterKey.value = key;
  if (aiModel && aiModel.value !== model) aiModel.value = model;
  if (openrouterModel && openrouterModel.value !== model) openrouterModel.value = model;
...
function saveOpenRouterConfig(baseVal, keyVal, modelVal, showNotification = true) {
  haptic(15);
  const current = getOpenRouterInputValues();
  const base = (baseVal !== undefined && baseVal !== null ? baseVal : current.base).trim() || 'https://openrouter.ai/api/v1';
  const key = (keyVal !== undefined && keyVal !== null ? keyVal : current.key).trim();
  const model = (modelVal !== undefined && modelVal !== null ? modelVal : current.model).trim() || 'openai/gpt-4o-mini';
  ...
  try {
    localStorage.setItem('cc_openrouter_base', base);
    if (key) {
      localStorage.setItem('cc_openrouter_key', key);
    } else {
      localStorage.removeItem('cc_openrouter_key');
    }
    localStorage.setItem('cc_openrouter_model', model);
  } catch (err) {
    console.warn('localStorage error:', err);
  }

  syncOpenRouterInputs();
  updateGatewayStatusBadge();
  ...
  if (showNotification) {
    showToast(key ? '🔒 OpenRouter Key saved in browser storage' : 'OpenRouter API Key cleared');
  }
}

function clearOpenRouterConfig() {
  haptic(15);
  state.aiAgent.apiKey = '';
  try {
    localStorage.removeItem('cc_openrouter_key');
  ...
  const r1 = document.getElementById('openrouter-modal-test-result');
  const r2 = document.getElementById('ai-sheet-test-result');
  [r1, r2, dom.openrouterModalTestResult, dom.aiSheetTestResult].forEach(el => {
    if (el) {
  ...
  showToast('OpenRouter API Key disconnected from browser');
}

async function testOpenRouterConnection(targetResultEl, testBase, testKey, testModel) {
  haptic(10);
  const current = getOpenRouterInputValues();
  ...
  if (!key) {
    if (targetResultEl) {
      ...
      targetResultEl.textContent = '❌ Please enter an OpenRouter API Key first.';
      targetResultEl.classList.remove('hidden');
    }
    showToast('Enter OpenRouter API Key first');
    return;
  }

  if (targetResultEl) {
    targetResultEl.className = 'api-test-result';
    targetResultEl.textContent = `⏳ Testing connection to OpenRouter (${model})...`;
    targetResultEl.classList.remove('hidden');
  }
  ...
  try {
    ...
    if (targetResultEl) {
      ...
      targetResultEl.textContent = `✅ Success! Authenticated with OpenRouter (${model}). Latency: ${elapsed}ms.`;
      targetResultEl.classList.remove('hidden');
    }
    // Auto-save the verified working key!
    saveOpenRouterConfig(base, key, model, false);
    showToast(`✅ OpenRouter connected (${elapsed}ms) & saved`);
  } catch (err) {
    if (targetResultEl) {
      targetResultEl.className = 'api-test-result fail';
...
function initAiAgent() {
  ...
  const setupKeyInput = (id) => {
    ...
    const onKeyInput = () => {
      ...
      const otherId = id === 'ai-key-input' ? 'openrouter-modal-key-input' : 'ai-key-input';
      ...
      try {
        if (val) {
          localStorage.setItem('cc_openrouter_key', val);
        } else {
          localStorage.removeItem('cc_openrouter_key');
        }
      } catch (_) {}
    ...
    el.addEventListener('change', () => {
      onKeyInput();
      if (el.value.trim()) {
        showToast('🔒 OpenRouter key stored in browser');
      }
    });
  ...
  setupKeyInput('openrouter-modal-key-input');
  ...
  const setupBaseInput = (id) => {
    ...
    if (!el) return;
    const onBaseInput = () => {
      const val = el.value.trim() || 'https://openrouter.ai/api/v1';
      state.aiAgent.gatewayUrl = val;
      const otherId = id === 'ai-gateway-base-input' ? 'openrouter-modal-base-input' : 'ai-gateway-base-input';
      const other = document.getElementById(otherId);
      if (other && other.value !== el.value) {
        other.value = el.value;
      }
      try { localStorage.setItem('cc_openrouter_base', val); } catch (_) {}
    };
    el.addEventListener('input', onBaseInput);
    el.addEventListener('change', onBaseInput);
  };
  setupBaseInput('ai-gateway-base-input');
  setupBaseInput('openrouter-modal-base-input');
  ...
  const setupModelInput = (id) => {
    ...
    if (!el) return;
    const onModelInput = () => {
      const val = el.value.trim() || 'openai/gpt-4o-mini';
      state.aiAgent.model = val;
      const otherId = id === 'ai-model-input' ? 'openrouter-modal-model-input' : 'ai-model-input';
      const other = document.getElementById(otherId);
      if (other && other.value !== el.value) {
        other.value = el.value;
      }
      ...
      try { localStorage.setItem('cc_openrouter_model', val); } catch (_) {}
      updateGatewayStatusBadge();
    };
  ...
  setupModelInput('openrouter-modal-model-input');
  ...
  document.querySelectorAll('.ai-model-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      const m2 = document.getElementById('openrouter-modal-model-input');
      if (m1) m1.value = m;
      if (m2) m2.value = m;
      document.querySelectorAll('.ai-model-chip').forEach(c => c.classList.toggle('active', c.dataset.model === m));
      try { localStorage.setItem('cc_openrouter_model', m); } catch (_) {}
      updateGatewayStatusBadge();
    });
  });
  ...
  document.querySelectorAll('.ai-strat-pill').forEach(pill => {
    ...
    pill.addEventListener('click', () => {
      ...
      try { localStorage.setItem('cc_openrouter_strat', s); } catch (_) {}
    });
  });
  ...
  if (dom.btnToggleOpenRouterModalVis && dom.openrouterModalKeyInput) {
    dom.btnToggleOpenRouterModalVis.addEventListener('click', () => {
      const isPass = dom.openrouterModalKeyInput.type === 'password';
      dom.openrouterModalKeyInput.type = isPass ? 'text' : 'password';
      dom.btnToggleOpenRouterModalVis.textContent = isPass ? '🔒' : '👁';
    });
  }

  // Unified API Sheet Tab Switching
  if (dom.tabBtnBinance && dom.tabBtnOpenRouter) {
    dom.tabBtnBinance.addEventListener('click', () => {
      haptic(10);
      dom.tabBtnBinance.classList.add('active');
      dom.tabBtnOpenRouter.classList.remove('active');
    ...
    dom.tabBtnOpenRouter.addEventListener('click', () => {
      haptic(10);
      dom.tabBtnOpenRouter.classList.add('active');
  ...
  // Save OpenRouter Config (Unified API Sheet)
  const openrouterModalSaveBtn = document.getElementById('btn-openrouter-modal-save') || dom.btnOpenRouterModalSave;
  if (openrouterModalSaveBtn) {
    openrouterModalSaveBtn.addEventListener('click', () => {
      saveOpenRouterConfig();
      closeAllSheets();
    });
  }

  // Disconnect OpenRouter (Unified API Sheet)
  const openrouterModalClearBtn = document.getElementById('btn-openrouter-modal-clear') || dom.btnOpenRouterModalClear;
  if (openrouterModalClearBtn) {
    openrouterModalClearBtn.addEventListener('click', clearOpenRouterConfig);
  }

  // Test OpenRouter (Unified API Sheet)
  const openrouterModalTestBtn = document.getElementById('btn-openrouter-modal-test') || dom.btnOpenRouterModalTest;
  if (openrouterModalTestBtn) {
    openrouterModalTestBtn.addEventListener('click', () => {
      const resEl = document.getElementById('openrouter-modal-test-result') || dom.openrouterModalTestResult;
      testOpenRouterConnection(resEl);
    });
  }

  // Save OpenRouter Config (AI Bottom Sheet)
  ...
  // Disconnect OpenRouter (AI Bottom Sheet)
  const clearAiConfigBtn = document.getElementById('btn-clear-ai-config') || dom.btnClearAiConfig;
  if (clearAiConfigBtn) {
    clearAiConfigBtn.addEventListener('click', clearOpenRouterConfig);
  }

  // Test OpenRouter (AI Bottom Sheet)
  ...
  if (dom.btnAiRunCustom) {
    dom.btnAiRunCustom.addEventListener('click', () => {
      ...
      if (prompt) {
        try { localStorage.setItem('cc_openrouter_prompt', prompt); } catch (_) {}
      }
...
window.syncOpenRouterInputs = syncOpenRouterInputs;
window.saveOpenRouterConfig = saveOpenRouterConfig;
window.clearOpenRouterConfig = clearOpenRouterConfig;
